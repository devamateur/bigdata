...
f = open('week04/age.csv', encoding='cp949')
data = csv.reader(f)
for row in data :
    print(row)
...
# pandas 프로그래밍
import pandas as pd
mydata=pd.read_csv("week04/age.csv", encoding='cp949')
print( mydata[mydata['행정구역'].isin(["신도림"])])
print(mydata[mydata['행정구역'].str.contains('신도림')].iloc[:,3:])  #iloc[인덱스, 열]
...
f = open('week04/age.csv', encoding='cp949')  # 파일 다시 열
data = csv.reader(f)
result = []
print(result)
for row in data :
