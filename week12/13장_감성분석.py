...
# ## 2. 데이터 준비 및 탐색
# ### 2-1) 훈련용 데이터 준비
# #### (1) 훈련용 데이터 파일 로드 -> 사람이 label에 긍정/부정 여부 표시(긍정은1, 부정은 0)
...
tfidf.fit(nsmc_train_df['document'])  # train 데이터로 tfidf 훈련
...
# 0) 입력 텍스트에 대한 전처리 수행
st = re.compile(r'[ㄱ-ㅣ가-힣]+').findall(st)  # 한글을 제외한 나머지 문자 제거
print(st)
st = [" ".join(st)]
print(st)
