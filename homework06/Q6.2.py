...
def getTourismStatsItem(yyyymm, national_code, ed_cd):    
    ...
    # print(retData)  # service key is not registered error
    if (retData == None):
        return None
    else:
...
def main():
    ...
    visitors_df = result_df['입국자 수']
    
    plt.figure(figsize=(16, 8))
    plt.bar(range(len(result_df['입국연월'])), visitors_df)
    plt.xticks(range(len(result_df['입국연월'])), result_df['입국연월'], rotation=90)
    plt.yticks([200000, 400000, 600000, 800000])
    plt.title('Visitors')
    plt.xlabel('yyyymm')
    plt.ylabel('persons', rotation=90)
    plt.show()
