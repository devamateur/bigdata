# -*- coding: utf-8 -*-
"""
Created on Mon Apr 11 18:34:55 2022
...
from konlpy.tag import Okt; t = Okt()   # 한국어 자연어 처리를 위한 형태소 분석기
...
def strip_e(st):
    # re.compile(): 정규식을 여러 번 사용할 때, 여기서는 sub()을 여러 번 사용
    RE_EMOJI = re.compile('[\U00010000-\U0010ffff]', flags=re.UNICODE)
    
    # r'': 따옴표에 포함된 내용을 그대로 문자열로 반환, 여기서는 공백문자열
    return RE_EMOJI.sub(r'', st)
content_text=strip_e(content_text)
tokens_ko = t.morphs(content_text)   # Okt.morphs(): 형태소 추출
tokens_ko = t.nouns(content_text)    # Okt.nouns(): 명사 추출
tokens_ko
ko = nltk.Text(tokens_ko)   
print(len(ko.tokens))          # 토큰 전체 갯수
print(len(set(ko.tokens)))     # 토큰 unique 갯수(중복된 값 제외)
ko = nltk.Text(tokens_ko)
ko.vocab().most_common(100)    # 빈도수가 가장 많은 단어 100개
