...
retail_df = retail_df[retail_df['CustomerID'].notnull()]   # CustomerID가 null이 아닌 경우만

# 'CustomerID' 자료형을 정수형으로 변환
retail_df['CustomerID'] = retail_df['CustomerID'].astype(int)

retail_df.info()
print(retail_df.isnull().sum())  # null인 데이터가 없음을 확인
...
# #### - 제품 수, 거래건 수, 고객 수 탐색
# 컬럼이름.value_counts(): 해당 컬럼의 데이터 개수
...
best_cluster = 4   # 실루엣 분석 결과 1에 가장 가까우므로(0.309)
...
# 클러스터별 분석
customer_cluster_summary_df = customer_cluster_df.drop(['CustomerID'], axis=1, inplace=False).groupby('ClusterLabel').mean()
customer_cluster_summary_df.to_csv('./Online_Retail_Customer_Cluster_Summary.csv')
