...
areaname = df.iloc[1:, 0]
df = df[1:]

# 동 이름 입력받기
name = input('원하는 지역의 이름(읍면동 단위)을 입력해주세요 : ')

new_areaname = df[0].str.split(' ').tolist()  # 행정구역
dong = []  # 동 이름만 저장
#home = [] # 입력받은 지역의 데이터 저장
#away = [] # 다른 지역들의 데이터 저장
#result_list = [] # 최종적으로 가장 비슷한 지역들을 저장

peoples_byage = df.iloc[:, 3:].astype('int64')
total = df.iloc[:, 2].astype('int64')
...
for i in range(len(new_areaname)):
    ...
    #if(dong[i][0] == name):
total[1]
for i in range(1, len(peoples_byage)):
    peoples_byage = peoples_byage.iloc[i, :] / total[i]
