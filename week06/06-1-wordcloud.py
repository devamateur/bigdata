...
for i in range(len(json_data)):
    file.write(json_data[i]['title'])  # json파일에서 배열의 title을가 져와 텍스트 파일에 저장
...
spwords = set(STOPWORDS)   # 불용어에서 중복 제거

content_text = open("textfile.txt", 'r', encoding='utf-8').read() # 텍스트 파일에 제목 잘 저장됨

def strip_e(st):
    RE_EMOJI = re.compile('[\U00010000-\U0010ffff]', flags=re.UNICODE) # BMP영역 이외 문자를 제거
    return RE_EMOJI.sub(r'', st)
content_text=strip_e(content_text)
tokens_ko = t.morphs(content_text)
tokens_ko = t.nouns(content_text)  # 명사 저장
tokens_ko
